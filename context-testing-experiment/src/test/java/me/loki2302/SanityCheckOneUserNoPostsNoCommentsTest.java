...
public class SanityCheckOneUserNoPostsNoCommentsTest {
    @Autowired
    ...
    @Test
    public void thereIsOneUserNoPostsAndNoComments() {
        assertEquals(1, userRepository.count());
        assertEquals(0, postRepository.count());
        assertEquals(0, commentRepository.count());

        assertNotNull(predefinedUser.id);

        User theOnlyUser = userRepository.findAll().get(0);
        assertEquals(predefinedUser.id, theOnlyUser.id);
    }
}
